...
public:
    void addPoint(mbgl::Point<int64_t>& point_, bool new_segment = false) {
        if(new_segment || segments.size() == 0) {
            LineRoutePlanSegment* segment = new LineRoutePlanSegment();
            segment->addPoint(point_);
