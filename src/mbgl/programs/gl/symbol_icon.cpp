...
//    static const char* navFragment(const char* shaderSource) { return shaderSource + fragmentOffset; }
    ...
    vec2 unpack_float(const float packedValue) {
        ...
        int v0=packedIntValue/256;return
        vec2(v0,packedIntValue-v0*256);
    }
    ...
    vec2 get_pattern_pos(const vec2 pixel_coord_upper,
                         ...
                         const float tile_units_to_pixels,
                         const vec2 pos) {
    ...
    size*=perspective_ratio;float fontScale=u_is_text ? size/24.0 : size;
