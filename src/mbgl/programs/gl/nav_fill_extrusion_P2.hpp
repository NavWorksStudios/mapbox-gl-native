...
//        uniform float u_focus_region;
        ...
        void main() {
            ...
            float diffuse = dot(norm, lightDir) * .4;
    
            // Specular Lighting
            const float indensity = .3; // 强度
            const float shininess = .01; // 反射率
            ...
            const vec3 specTone = vec3(1., .9, .4);
            vec4 specLight = vec4(1.) + vec4(specTone * specular, 1.);
            v_color = color * vec4(u_lightcolor * (ambient + diffuse), 1.) * u_opacity * .95 * specLight;
        }
