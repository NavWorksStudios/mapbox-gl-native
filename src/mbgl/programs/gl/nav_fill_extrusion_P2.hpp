...
//        uniform float u_focus_region;
        ...
        void main() {
            ...
            const float shininess = .00001; // 反射率
            ...
            const vec3 basecolor = vec3(.97, .97, .8);
            const vec3 specularcolor = vec3(.9, .94, .94);
    
            v_color = color * vec4(u_lightcolor * basecolor * (ambient) + specularcolor * (diffuse + specular), 1.) * u_opacity;
            v_color.a *= .95;
        }
