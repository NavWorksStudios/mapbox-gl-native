...
RenderBackgroundLayer::RenderBackgroundLayer(Immutable<style::BackgroundLayer::Impl> _impl)
    ...
    bindToPalette(baseImpl->id, "background-color", unevaluated.get<BackgroundColor>().value);
}
...
void RenderBackgroundLayer::render(PaintParameters& parameters) {
    ...
    if (!evaluated.get<BackgroundPattern>().to.empty()) {
        ...
        uint32_t i = 0;
        util::tileCover(tileIDs, util::strategy::standard(parameters.state), parameters.state, parameters.state.getIntegerZoom());
