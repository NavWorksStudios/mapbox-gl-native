    ...
    if (overscaledZoom >= zoomRange.min) {
        ...
        if (parameters.mode == MapMode::Continuous && type != style::SourceType::GeoJSON && type != style::SourceType::Annotations) {
            ...
            if (panZoom < idealZoom) {
                util::tileCover(panTiles, util::strategy::standard(parameters.transformState), parameters.transformState, panZoom);
            }
        }

        util::tileCover(idealTiles, util::strategy::standard(parameters.transformState), parameters.transformState, idealZoom, tileZoom);
...
std::unordered_map<nav::stringid, std::vector<Feature>> TilePyramid::queryRenderedFeatures(
    const ScreenLineString& geometry, const TransformState& transformState,
    const std::unordered_map<nav::stringid, const RenderLayer*>& layers, const RenderedQueryOptions& options,
