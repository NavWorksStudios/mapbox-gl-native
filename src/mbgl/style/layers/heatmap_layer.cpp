...
optional<Error> HeatmapLayer::setPropertyInternal(const std::string& name, const Convertible& value) {
    const auto it = layerProperties.find(name.c_str());
    if (it == layerProperties.end()) return Error{std::string(name) + " <- HeatmapLayer doesn't support this property"};
    ...
    return Error{std::string(name) + " <- HeatmapLayer doesn't support this property"};
}
