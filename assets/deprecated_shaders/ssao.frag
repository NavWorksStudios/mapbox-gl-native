...
void main()
{
  ...
  vec3 normal = texture2D(normTex, texCoord).xyz;
  // Scale and bias
  normal = (2.0 * normal) - vec3(1.0);
  normal = normalize(normal);

  // Construct our rotation matrix (used to transform sample offsets) based on a random vector lookup
  vec3 randomVector = texture2D(randomTex, texCoord * randomTexCoordScale).xyz;
  ...
  for (int i = 0; i < 16; i++) {
    ...
    float lookupDepth = texture2D(depthTex, screenSample).x;
