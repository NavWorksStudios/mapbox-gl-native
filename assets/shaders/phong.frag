...
void main()
{
  ...
  float diffuseC = dot(normal, normalize(lPosition - positionView));
  vec3 diffuse = max(0.0, diffuseC) * kDiffuse * lDiffuse;

  vec3 specular = vec3(0.0);
  float specPreExp = 0.0;
  if (diffuseC >= 0.0) {
    vec3 reflection = reflect(normalize(positionView - lPosition), normal);
    specPreExp = dot(normalize(reflection), normalize(-positionView));
